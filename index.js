...
app.post('/addCard', async function (req, res) {
  try {
    ...
    console.log(
      `Card titled ${req.body.listName} was successfully added to the collection of lists!`
    );
...
app.post('/deleteCard', function (req, res) {
  ...
  List.findByIdAndRemove(listToDelete)
    .then(function () {
      console.log(
        `Card was successfully removed from the collection of lists!`
      );
      res.redirect('/');
    })
    .catch(function (err) {
...
app.post('/add', async function (req, res) {
  ...
  try {
    ...
    console.log(
      `"Number of documents modified: ${result.modifiedCount}. Task "${
        newItem.substring(0, 1).toUpperCase() +
        newItem.substring(1, newItem.length)
      }" was added to "${listName}" task list!`
    );
...
const aboutContent = `I'm a dedicated Junior Software Engineer eager to contribute my skills to innovative projects. 
...
const aboutAppContent = `This is a ToDoList App that uses CSS/HTML and EJS for the front-end, uses Node.JS & Express.JS as a back-end, and MongoDB as a database. There are no external front-end libraries used, except few of the icons. 
